...
| 🌐 **Mesh Discovery** | Analyze captured packets to discover active mesh nodes — shows node names, last seen time, packet count, average SNR/RSSI, channels used, portnums, average hop limit, and Direct RF vs MQTT badges. Configurable time window (1 min to 1 hour) and result limit. Available in the Packet Inspector Stats panel. |
...
| � **Bidirectional Favorite Sync** | Favorites are now synced bidirectionally: UI favorites sync to the device, and device favorites (set via Meshtastic Android, etc.) automatically sync to the UI every 60 seconds, keeping all clients in sync |
