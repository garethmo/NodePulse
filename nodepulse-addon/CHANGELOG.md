...
- **Node deduplication and ID canonicalization** — Fixed a bug causing duplicate nodes to appear in the node list, grid, and on the map. Canonicalized node IDs (handling decimal string IDs, uppercase hex, and integer keys) across the backend (`connection.py`) and frontend (`app.js`, `map.js`). `_get_nodes_sync` and `_load_nodes` now enforce strict unique ID sets, preventing duplicate entries from radio DB merges, stale re-injection, or unnormalized IDs.
- **Co-located map markers and icon separation** — Fixed an issue where co-located nodes (sharing identical coordinates or within 25m) piled all marker icons on top of each other, hiding all but the topmost marker. Marker icons and permanent labels now vertically stack in sync with matching offsets (`yOffset`) and custom icon anchors so that every co-located node (Gateway, Router, Client, Tracker) has its own distinct, clickable icon aligned alongside its label.
...
## [1.23.1] - 2026-08-31
### Added
- **Bidirectional Favorite Sync** — Implemented two-way synchronization between NodePulse UI favorites and the device's NodeDB favorites. Favorites set via other apps (Meshtastic Android, etc.) are now automatically synced to the NodePulse UI every 60 seconds, and UI favorites continue to sync to the device.
- **Periodic Favorite Sync Loop** — Added background task that polls the device's NodeDB for favorites every 60 seconds, ensuring the UI stays in sync with changes made via other clients.
- **Device Favorite Reading** — Added `_sync_favorites_from_device()` method to read favorites from the device's `localNode.favorites` and merge them with local UI favorites.

### Fixed
- **Message Archive Race Conditions** — Fixed potential race conditions in message archive loading/saving by properly using `_persist_lock` for all archive file operations and implementing deduplication logic to prevent duplicate messages in archive files.
- **Archive Deduplication** — Implemented ID-based deduplication when merging messages into archive files to prevent duplicate message accumulation over time.
